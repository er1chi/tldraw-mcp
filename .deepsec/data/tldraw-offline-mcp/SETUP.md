# Agent setup for `tldraw-offline-mcp`

This is a deepsec scanning workspace. Project `tldraw-offline-mcp` was just registered
(target: `..`). Setup is incomplete — `data/tldraw-offline-mcp/INFO.md`
...
2. **Fill in `data/tldraw-offline-mcp/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id tldraw-offline-mcp
pnpm deepsec process --project-id tldraw-offline-mcp
```
