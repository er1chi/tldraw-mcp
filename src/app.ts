...
export function createApp(config: AppConfig): Hono {
  ...
  app.get('/healthz', (c) => c.json({ status: 'ok', service: 'tldraw-offline-mcp' }))

  app.use('/readyz', bearerAuth(config))
  app.get('/readyz', async (c) => {
    try {
      return c.json({ status: 'ready', app: await canvas.readiness(c.req.raw.signal) })
    } catch (error) {
      return c.json({ status: 'not-ready', reason: error instanceof Error ? error.message : String(error) }, 503)
    }
  })
  ...
  app.onError((error, c) => {
    console.error(JSON.stringify({ level: 'error', requestId: c.req.header('x-request-id'), path: c.req.path, message: error.message }))
